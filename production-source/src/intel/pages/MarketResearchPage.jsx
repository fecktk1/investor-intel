...
export function ResearchTable({ rows, capability, onOpen, t }) {
  ...
  else if (capability === 'liquidationExchanges') columns = ['1h','4h','24h'].map(period => column(`liquidations_${period}`, `${period} liquidations (USD)`, r => fmtVol(q(r)[`total_liquidations_${period}`]))).concat([column('quote_observed','Observed',r => q(r).last_updated ? new Date(q(r).last_updated).toLocaleString() : 'Unreported',false)])
  ...
  else if (capability.startsWith('rwa')) columns = [column('type', 'Asset type', r => String(r.asset_type || '—').replaceAll('_',' '), false), column('price', 'Tokenized price (USD)', r => fmtPrice(r.average_tokenized_price ?? q(r).average_tokenized_price)), column('value', 'Tokenized value (USD)', r => fmtVol(r.tokenized_market_cap ?? q(r).tokenized_market_cap)), column('volume', '24h volume (USD)', r => fmtVol(r.tokenized_volume_24h ?? q(r).tokenized_volume_24h)), column('quote_observed', 'Quote observed', r => q(r).last_updated ? new Date(q(r).last_updated).toLocaleString() : 'No dated quote', false)]
