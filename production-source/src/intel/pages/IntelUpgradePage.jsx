...
export default function IntelUpgradePage() {
  ...
  // The free column's copy lives in the intel namespace with the rest of the
  // free-tier wording, so it is read from there rather than falling back to
  // English in every other locale.
  const { t: ti } = useTranslation('intel')
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="grid sm:grid-cols-4 gap-3 mb-8">
          <div className="relative text-left rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-1)] p-4">
            <p className="text-sm font-bold text-white mb-1">{ti('intel_upgrade.free_label', { defaultValue: 'Free' })}</p>
            <p className="text-2xl font-black text-white mb-1">
              {ti('intel_upgrade.free_price', { defaultValue: '$0' })}
              <span className="text-xs font-medium text-gray-500"> {ti('intel_upgrade.free_note', { defaultValue: 'no card, no trial clock' })}</span>
            </p>
            <p className="text-[11px] text-gray-400 mb-2">
              {ti('intel_upgrade.free_body', { defaultValue: 'The same product, not a smaller one. Everything already computed once for everyone stays open to you.' })}
            </p>
            <p className="text-[11px] font-semibold text-gray-300">{ti('intel_upgrade.free_open_title', { defaultValue: 'Open on free' })}</p>
            <ul className="space-y-1 text-[11px] text-gray-400">
              <li>{ti('intel_upgrade.free_open_boards', { defaultValue: 'Every market board and asset page' })}</li>
              <li>{ti('intel_upgrade.free_open_regime', { defaultValue: 'Market regime figures' })}</li>
              <li>{ti('intel_upgrade.free_open_captures', { defaultValue: 'Every recorded capture' })}</li>
