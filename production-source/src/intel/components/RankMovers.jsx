...
const assetHref = (sym) => `/intel/markets/${encodeURIComponent(String(sym || '').toUpperCase())}`

function MoverRow({ m, climb }) {
  return (
    <Link to={assetHref(m.symbol)} className="flex items-center justify-between gap-2 px-2 py-1 rounded hover:bg-[var(--bg-2)] transition-colors">
...
export default function RankMovers({ days = 7 }) {
  ...
  useEffect(() => {
    let alive = true
    loadRankMovers(supabase, { days }).then((d) => { if (alive) setData(d) }).catch(() => { if (alive) setData(null) })
    return () => { alive = false }
  }, [supabase, days])
  ...
  return (
    <section className="grid gap-3 sm:grid-cols-2">
      <div className="card p-3 space-y-1">
        ...
        {data.climbers.length ? data.climbers.map((m) => <MoverRow key={m.symbol} m={m} climb />) : <div className="text-[12px] text-[var(--fg-5)] px-2 py-1">{t('markets.no_rank_moves', { defaultValue: 'No notable moves' })}</div>}
      </div>
      <div className="card p-3 space-y-1">
        ...
        {data.fallers.length ? data.fallers.map((m) => <MoverRow key={m.symbol} m={m} climb={false} />) : <div className="text-[12px] text-[var(--fg-5)] px-2 py-1">{t('markets.no_rank_moves', { defaultValue: 'No notable moves' })}</div>}
      </div>
    </section>
  )
}
