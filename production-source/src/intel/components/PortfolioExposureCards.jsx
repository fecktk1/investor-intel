...
export default function PortfolioExposureCards({ portfolioId }) {
  const { supabase } = useSupabase()
  const [exp, setExp] = useState(null)

  useEffect(() => {
    let alive = true
    if (!portfolioId) { setExp(null); return }
    supabase.rpc('portfolio_exposure', { p_portfolio_id: portfolioId })
      .then(({ data, error }) => { if (alive) setExp(error ? null : data) })
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="card p-3 space-y-1.5">
            ...
            {chains.slice(0, 5).map((c) => (
              <div key={c.chain} className="flex items-center gap-2 text-[12px]">
                ...
                <div className="flex-1 h-1.5 rounded bg-[var(--bg-2)] overflow-hidden"><div className="h-full bg-[var(--accent)]" style={{ width: `${Math.min(100, c.pct || 0)}%` }} /></div>
        ...
        {narratives.length > 0 && (
          <div className="card p-3 space-y-1.5">
            <div className="text-[10px] text-[var(--fg-4)] uppercase">Narrative exposure</div>
            {narratives.slice(0, 5).map((n) => (
              <div key={n.slug} className="text-[12px] space-y-0.5">
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  {n.signal_class && <span className={`chip text-[9px] ${SIG_CLS[n.signal_class] || ''}`}>{n.signal_class}</span>}
                ...
                {Array.isArray(n.clarity_labels) && n.clarity_labels.slice(0, 1).map((l) => {
                  const m = clarityMeta(l.key); return <span key={l.key} className={`chip text-[9px] ${m.cls}`} title={m.why}>{m.label}</span>
                })}
              </div>
            ))}
          </div>
        ...
        <div className="card p-3 space-y-2 text-[12px]">
          ...
          {bc?.symbol && (
            <div><span className="text-[var(--fg-5)]">Biggest move since last sync: </span>
              ...
              <span className={bc.day_change_usd >= 0 ? 'text-emerald-400' : 'text-red-400'}>{bc.day_change_usd >= 0 ? '+' : ''}{fmtUsd(Math.abs(bc.day_change_usd))} ({bc.day_change_pct != null ? `${bc.day_change_pct > 0 ? '+' : ''}${bc.day_change_pct}%` : '—'})</span>
            </div>
          )}
        </div>
      </div>
            ...
            <div className="card p-3 space-y-1.5">
              ...
              {signals.slice(0, 4).map((s, i) => (
                <div key={i} className="text-[12px] leading-snug">
                  ...
                  <span className={`chip text-[9px] ml-1.5 ${SIG_CLS[s.direction] || ''}`}>{s.direction}</span>
