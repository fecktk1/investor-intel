...
export default function RwaTerms({subject,token,observations,at,onPrepareReceipt}){
 ...
 // No structured terms recorded for this token: a stated state rather than a
 // missing panel. Issuer terms come from editorial source reviews, not from a
 // capture job, and a review is re-run when there is a reason to, not on a timer.
 if(!result.facts.length)return <p role="status" className="intel-analysis-caption my-3">{t('investigation.terms_not_recorded',{defaultValue:'No unit or redemption terms have been recorded for this token yet. Issuer sources are re-read when a source is known to have changed or when a re-review is asked for, and the evidence table below lists what is recorded.'})}</p>
 ...
 return <section aria-label={t('investigation.terms_calculation',{defaultValue:'Token units and redemption conditions'})} className="py-3 border-y border-[var(--border-default)] my-3">
  ...
  {result.state!=='reviewed'?<p role="status">{t('investigation.terms_expired',{defaultValue:'These terms were withdrawn by a later review and need a new source review before calculating.'})}</p>:<>
