...
export default function TradePlanForm({ theses = [], defaultThesisId = null, onSave, onCancel, busy }) {
  ...
  const [f, setF] = useState({
    ...
    planned_size_usd: '', risk_amount: '', time_stop: '', invalidation_reason: '', pre_notes: '', emotion: '', tags: '',
  })
  ...
  const submit = () => {
    ...
    onSave({
      thesis_id: f.thesis_id || null, symbol: f.symbol.trim().toUpperCase(), chain: f.chain, direction: f.direction,
    ...
    <div className="card p-4 space-y-3">
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        <label className="block"><span className="text-[10px] text-[var(--fg-4)]">{t('journal.trade.thesis', { defaultValue: 'Link thesis (optional)' })}</span>
          <select className="select w-full" value={f.thesis_id} onChange={(e) => set({ thesis_id: e.target.value })}>
      ...
      <div className="flex justify-end gap-2">
        ...
        <button onClick={submit} disabled={busy || !f.symbol.trim()} className="btn btn--primary btn--sm disabled:opacity-50">{t('journal.trade.save_plan', { defaultValue: 'Save plan' })}</button>
      </div>
    </div>
  )
}
