...
function ChartShareControlsBody({context,snapshot,autoOpen=false}){
 const [open,setOpen]=useState(autoOpen),[audience,setAudience]=useState('owner'),[days,setDays]=useState(7),[selected,setSelected]=useState([]),[shares,setShares]=useState([]),[page,setPage]=useState(0),[hasMore,setHasMore]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState(null),[result,setResult]=useState(null),[copied,setCopied]=useState(false)
