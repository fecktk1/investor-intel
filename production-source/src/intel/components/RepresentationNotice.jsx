...
export default function RepresentationNotice({canonicalAssetKey,record,asOf}){
 ...
 return <aside aria-label="Issuer deployment notice" className="border-l-2 border-[var(--accent)] pl-3 my-3 text-sm">
  <p><strong>{review.issuer} · {review.network} deployment notice</strong>{review.status==='review_expired'?' · Review withdrawn; it needs re-reading':''}</p>
  <p>{review.summary}</p>
  <details><summary>Source and dates</summary><p>Issuer effective date: <time dateTime={review.effectiveDate}>{review.effectiveDate}</time>. Time and timezone unreported.</p>
   <p>Source reviewed: <time dateTime={review.reviewedAt}>{review.reviewedAt}</time>. {review.reviewLapsedAt?<>Review withdrawn: <time dateTime={review.reviewLapsedAt}>{review.reviewLapsedAt}</time>.</>:'The review stays current until it is re-read or withdrawn.'}</p>
