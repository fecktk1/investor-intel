...
export default function RwaSessions({subject,records=[],observations=[],at,selectedToken,onTokenChange,onPrepareReceipt}) {
 ...
 return <section aria-label={t('investigation.rwa_clocks',{defaultValue:'Token, underlying and issuer clocks'})} className="intel-rwa-clocks">
  ...
  {review.state==='unavailable'?<p role="status">{t('investigation.issuer_missing',{defaultValue:'No reviewed issuer evidence was recorded for this token at the selected time. Select another token or inspect its issuer documentation.'})}</p>:<>
   {review.state==='review_expired'&&<p role="status">{t('investigation.issuer_expired',{defaultValue:'This source review has expired. The recorded summary remains visible; current hours and terms require re-verification.'})}</p>}
   {review.state==='partially_reviewed'&&<p role="status">{t('investigation.issuer_partially_reviewed',{defaultValue:'Some source reviews have expired. Only independently current facts are used for calculations and market clocks.'})}</p>}
   ...
   <p className="intel-analysis-caption">{t('investigation.independent_reviews',{defaultValue:'Each fact has its own source review and expiry below. A newer review does not refresh the other documents.'})}</p>
   ...
   <InvestigationTable rows={review.facts} pageSize={6} columns={[[t('investigation.issuer_fact',{defaultValue:'Issuer evidence'}),o=>o.metadata?.label],
    [t('investigation.recorded_summary',{defaultValue:'Recorded summary'}),o=><>{String(o.value??'')}{observationState(o,at)!=='known'&&<p>{t('investigation.fact_review_expired',{defaultValue:'Source review expired'})}</p>}</>],
    [t('investigation.source_review',{defaultValue:'Source / review'}),o=><><a className="intel-text-link" href={safeSourceUrl(o.sourceUrl)||undefined} target="_blank" rel="noreferrer">Issuer source</a><details><summary className="cursor-pointer text-xs">Review history</summary><small className="block">Reviewed {time(o.observedAt)}<br/>Recorded {time(o.recordedAt)}<br/>Review due {time(o.expiresAt)}</small></details></>]]}/>
  </>}
 </section>
}
