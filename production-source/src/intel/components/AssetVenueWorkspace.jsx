...
export default function AssetVenueWorkspace({canonicalKey}){const {org}=useProfile(),{user}=useSupabase();return <Workspace key={`${user?.id}:${org?.id}:${canonicalKey}`} canonicalKey={canonicalKey}/>}
function Workspace({canonicalKey}){
