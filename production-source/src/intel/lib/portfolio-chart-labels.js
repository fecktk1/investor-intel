import {usdUnit} from './market-format'
...
export function portfolioAxisFormatter(values){
 ...
 // Unit ladder is shared with market-format; axes deliberately stop short of the
 // K tier, because a portfolio in the thousands reads better in full dollars.
 const scale=usdUnit(maximum),thousands=scale.suffix==='K'
 const unit=thousands?1:scale.divisor,suffix=thousands?'':scale.suffix
 ...
 return value=>!Number.isFinite(value)?'':`${value<0?'-':''}$${format.format(Math.abs(value)/(compact?unit:1))}${compact?suffix:''}`
}
