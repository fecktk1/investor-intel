-- Rank category membership using narrow keys; attach full rows only for three leaders.
-- Existing SECURITY INVOKER, membership, entitlement, provider and private watchlist scope remain unchanged.
...
BEGIN
 ...
 category_leaders AS (SELECT c.category,jsonb_agg(to_jsonb(b)||jsonb_build_object('category',c.category,'position',c.position,'members',c.members) ORDER BY c.position) AS leaders,max(c.mover_score) AS score
