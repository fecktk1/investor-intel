-- Exact selected-portfolio relevance; the original all-interest feed remains available.
CREATE OR REPLACE FUNCTION public.intel_selected_portfolio_signals(p_org_id uuid, p_portfolio_id uuid, p_subject_type text DEFAULT NULL::text, p_chains text[] DEFAULT NULL::text[], p_limit integer DEFAULT 60)
...
BEGIN
  ...
  IF p_portfolio_id IS NULL OR NOT EXISTS(SELECT 1 FROM public.investor_portfolios p WHERE p.id=p_portfolio_id AND p.org_id=p_org_id AND p.user_id=auth.uid()) THEN RAISE EXCEPTION 'Portfolio unavailable' USING ERRCODE='42501';END IF;
  RETURN QUERY
  WITH
  identity AS MATERIALIZED (
    ...
    SELECT coalesce(array_agg(DISTINCT k),'{}') keys FROM public.watchlist_items wi
    ...
    FROM public.investor_portfolios p JOIN public.investor_portfolio_holdings h ON h.portfolio_id=p.id AND h.org_id=p.org_id AND h.user_id=p.user_id
    CROSS JOIN LATERAL unnest(ARRAY[h.canonical_asset_key] || app_private.intel_issuer_market_keys(h.canonical_asset_key) ||
      ...
      ELSE '{}'::text[] END) k(key)
    WHERE p.id=p_portfolio_id AND p.org_id=p_org_id AND p.user_id=auth.uid() AND h.quantity>0 AND NOT coalesce(h.is_closed,false)
    GROUP BY k.key
  ),
  ...
  prof AS (  -- onboarding selections (single row, empty arrays if no profile)
    SELECT COALESCE((SELECT chains_of_interest FROM public.intel_user_profiles WHERE org_id = p_org_id AND user_id=auth.uid() LIMIT 1), '{}') AS chains,
           COALESCE((SELECT topics_of_interest FROM public.intel_user_profiles WHERE org_id = p_org_id AND user_id=auth.uid() LIMIT 1), '{}') AS topics
  )
  SELECT
  ...
  FROM public.intel_signal_state s
  ...
  WHERE hw.w IS NOT NULL AND (s.expires_at IS NULL OR s.expires_at > now())
...
END $function$;
REVOKE ALL ON FUNCTION public.intel_selected_portfolio_signals(uuid,uuid,text,text[],integer) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.intel_selected_portfolio_signals(uuid,uuid,text,text[],integer) TO authenticated;
