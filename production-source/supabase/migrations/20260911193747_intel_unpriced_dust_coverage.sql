-- Unknown unit price is unpriced even when an old value remains. Display-only; accounting totals are unchanged.
...
CREATE OR REPLACE FUNCTION public.intel_desk_positions(p_org_id uuid,p_portfolio_id uuid,p_hide_dust boolean DEFAULT false,p_pinned text[] DEFAULT '{}',p_page integer DEFAULT 0)
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY INVOKER SET search_path='' AS $$
