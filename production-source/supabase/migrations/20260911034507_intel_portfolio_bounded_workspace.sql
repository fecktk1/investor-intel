-- Bounded owner-only workspace reads. All accounting stays in the existing ledger.
CREATE INDEX IF NOT EXISTS iph_workspace_page ON public.investor_portfolio_holdings(portfolio_id, is_closed, current_value DESC NULLS LAST, id);
CREATE INDEX IF NOT EXISTS ips_workspace_page ON public.investor_portfolio_sources(portfolio_id, created_at, id);

CREATE FUNCTION public.intel_portfolio_page(p_org_id uuid,p_portfolio_id uuid,p_kind text DEFAULT 'holdings',p_view text DEFAULT 'open',p_page integer DEFAULT 0,p_limit integer DEFAULT 25,p_search text DEFAULT '',p_sort text DEFAULT 'value',p_hide_dust boolean DEFAULT false,p_export boolean DEFAULT false,p_revision text DEFAULT NULL)
 ...
 ELSE
  ...
  WITH filtered AS MATERIALIZED (
   SELECT h.* FROM public.investor_portfolio_holdings h WHERE h.portfolio_id=p_portfolio_id AND h.org_id=p_org_id AND h.user_id=auth.uid()
    ...
    AND (NOT coalesce(p_hide_dust,false) OR p_view='closed' OR NOT(coalesce(h.asset_class,'') NOT IN('native','stablecoin') AND (h.current_value IS NULL OR h.current_value<1)))
  ), page AS (
...
CREATE FUNCTION public.intel_portfolio_overview(p_org_id uuid,p_portfolio_id uuid)
RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY INVOKER SET search_path='' AS $$
...
BEGIN
 ...
 WITH scoped AS MATERIALIZED (
  SELECT h.*,coalesce(h.asset_class,'') NOT IN('native','stablecoin') AND (h.current_value IS NULL OR h.current_value<1) AS dust,
