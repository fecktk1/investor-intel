-- Markets screen: distance from the recorded high (CMC plan proposal 14).
-- Additive only. No authorization, provider, clock, membership or contract
-- change: every existing sort key keeps the direction it already had, every
-- existing column keeps its position, and the new reading is computed from rows
-- this workspace already recorded, so it costs zero provider credits.
--
-- HONESTY RULE. `intel_rank_history` began on 2026-09-14; the weekly backfill
-- is still filling Mondays behind it. The "high" here is therefore the high of
-- the RECORDED window, never an all-time high, and the window is published
-- alongside it (`recorded_window_days`, `recorded_high_date`) so the surface can
-- say what it measured against instead of implying a full price history.
SET LOCAL lock_timeout='5s';
SET LOCAL statement_timeout='60s';

-- Four derived columns on the catalogue. `intel_replace_market_catalog` names
-- its columns explicitly and lists none of these, so the 5-minute catalogue
-- rewrite leaves them alone on an ON CONFLICT update; a brand-new asset simply
-- starts null until the next nightly refresh.
ALTER TABLE public.market_assets
  ADD COLUMN IF NOT EXISTS recorded_high_price numeric CHECK (recorded_high_price IS NULL OR (recorded_high_price > 0 AND recorded_high_price <= 1e30)),
  ADD COLUMN IF NOT EXISTS recorded_high_date date,
  ADD COLUMN IF NOT EXISTS recorded_window_days integer CHECK (recorded_window_days IS NULL OR recorded_window_days >= 0),
  ADD COLUMN IF NOT EXISTS drawdown_pct numeric CHECK (drawdown_pct IS NULL OR (drawdown_pct <= 0 AND drawdown_pct >= -100));

-- Recompute the recorded high for the catalogue's top 1,000 in one statement.
--
-- The high is the highest price this workspace has RECORDED for the asset, and
-- the catalogue's own current price is one of the candidates: a live price above
-- every recorded day IS the high of the window, so the drawdown is 0 rather than
-- a positive number. `drawdown_pct` stays null below two recorded days, because
-- a single day cannot describe a distance from anything.
--
-- The scope is ordered exactly like the screen's canonical CTE, so the 1,000
-- rows refreshed here are the 1,000 rows the screen shows.
CREATE OR REPLACE FUNCTION public.intel_refresh_recorded_highs(p_provider text)
RETURNS jsonb LANGUAGE plpgsql SECURITY INVOKER SET search_path='' AS $highs$
DECLARE v_updated int;
BEGIN
 IF p_provider IS DISTINCT FROM 'coinmarketcap' THEN RAISE EXCEPTION 'Invalid recorded-high provider' USING ERRCODE='22023'; END IF;
 WITH scope AS (
  SELECT m.source_provider,m.provider_id,
   -- A NaN price compares greater than every float8, so the upper bound rejects
   -- it as well as an infinity; only a finite positive price is a candidate.
   CASE WHEN m.current_price>0 AND m.current_price<'Infinity'::float8 THEN m.current_price::numeric ELSE NULL END AS price,
   (m.as_of AT TIME ZONE 'UTC')::date AS price_date
  FROM public.market_assets m
  WHERE m.source_provider=p_provider AND m.in_current_catalog
  ORDER BY m.market_cap_rank,m.provider_id LIMIT 1000
 ), recorded AS (
  -- One index-ordered pass per asset over (provider,provider_id,snapshot_date).
  -- intel_rank_history_finite already rejects NaN and infinite prices, so a
  -- positive price here is a real one.
  SELECT s.source_provider,s.provider_id,s.price,s.price_date,h.high_price,h.high_date,COALESCE(h.days,0) AS days
  FROM scope s LEFT JOIN LATERAL (
   SELECT max(r.price) FILTER (WHERE r.price>0) AS high_price,
    (array_agg(r.snapshot_date ORDER BY r.price DESC,r.snapshot_date DESC) FILTER (WHERE r.price>0))[1] AS high_date,
    count(DISTINCT r.snapshot_date) AS days
   FROM public.intel_rank_history r
   WHERE r.provider=s.source_provider AND r.provider_id=s.provider_id
  ) h ON true
 ), computed AS (
  SELECT c.source_provider,c.provider_id,c.days,c.price,
   CASE WHEN c.price IS NOT NULL AND (c.high_price IS NULL OR c.price>=c.high_price) THEN c.price ELSE c.high_price END AS high_price,
   CASE WHEN c.price IS NOT NULL AND (c.high_price IS NULL OR c.price>=c.high_price) THEN c.price_date ELSE c.high_date END AS high_date
  FROM recorded c
 )
 UPDATE public.market_assets m SET
  recorded_high_price=c.high_price,
  recorded_high_date=c.high_date,
  recorded_window_days=c.days::int,
  -- LEAST keeps the column's meaning ("distance from the high") a non-positive
  -- number even if a rounding artefact ever lifted the ratio above one.
  drawdown_pct=CASE WHEN c.days>=2 AND c.price IS NOT NULL AND c.high_price IS NOT NULL AND c.high_price>0
   THEN LEAST(0::numeric,round((c.price/c.high_price-1)*100,6)) ELSE NULL END
 FROM computed c
 WHERE m.source_provider=c.source_provider AND m.provider_id=c.provider_id;
 GET DIAGNOSTICS v_updated=ROW_COUNT;
 RETURN jsonb_build_object('provider',p_provider,'refreshed',v_updated);
END $highs$;
REVOKE ALL ON FUNCTION public.intel_refresh_recorded_highs(text) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.intel_refresh_recorded_highs(text) TO service_role;

-- The reviewed provider-scoped projection, gaining the four recorded-high
-- columns only. CREATE OR REPLACE VIEW can only append columns, so they land
-- after num_market_pairs, the last column 20260914234442 appended.
DO $migration$
...
BEGIN
 ...
 -- 20260912203009 guard) and 'public' (how 20260914234442 guards). The hashes
 -- below are what 20260914234442 leaves behind; PGlite 0.3.14 reproduces both.
 ...
 PERFORM pg_catalog.set_config('search_path','',true);
 IF pg_catalog.md5(qualified_definition)<>'e15e8d007c574ca657f6eca08e707648'
  OR pg_catalog.md5(scoped_definition)<>'84e14a40993bc5618ee4a17eb8eb1eb3' THEN
...
r.flags,
r.num_market_pairs,
r.recorded_high_price,
r.recorded_high_date,
r.recorded_window_days,
r.drawdown_pct
...
CROSS JOIN LATERAL (
 WITH canonical AS (
         SELECT a.source_provider,
            ...
            a.derived,
            a.quote_batch_ids,
            a.recorded_high_price,
            a.recorded_high_date,
            a.recorded_window_days,
            a.drawdown_pct
           FROM LATERAL ( SELECT m.source_provider,
                    ...
                    m.derived,
                    m.quote_batch_ids,
                    m.recorded_high_price,
                    m.recorded_high_date,
                    m.recorded_window_days,
                    m.drawdown_pct,
         ...
         SELECT j.source_provider,
            ...
            j.derived,
            j.quote_batch_ids,
            j.recorded_high_price,
            j.recorded_high_date,
            j.recorded_window_days,
            j.drawdown_pct,
            j.profile_providers,
            j.latest_price,
...
-- Same screener, same authorization, same panels and the same twelve reviewed
-- sort keys. 'drawdown' is the thirteenth: its primary expression is
-- drawdown_pct, and it defaults ASCENDING — the deepest (most negative)
-- drawdown first — for the same reason 'rank' and 'losers' do, because the
-- informative end of this column is its low end. dir='desc' flips it to the
-- rows closest to their recorded high. Ties break by market cap, then identity.
...
BEGIN
 ...
 v_base:=CASE v_sort WHEN 'market_cap' THEN 'market_cap' WHEN 'volume' THEN 'volume_24h'
  ...
  WHEN 'drawdown' THEN 'drawdown_pct'
 ...
 v_tail:=CASE v_sort WHEN 'exchange_availability' THEN ',market_cap DESC NULLS LAST'
  ...
  WHEN 'multi_exchange_strength' THEN ',change_24h_pct DESC NULLS LAST'
  WHEN 'drawdown' THEN ',market_cap DESC NULLS LAST' ELSE '' END;
 ...
 v_direction:=CASE WHEN v_sort IN ('gainers','losers') OR v_dir='' THEN
   CASE WHEN v_sort IN ('rank','losers','drawdown') THEN 'asc' ELSE 'desc' END ELSE v_dir END;
