-- ============================================================
-- 183: Investor Intel — canonical asset key SQL helper
-- ============================================================
-- IMMUTABLE helper that mirrors _shared/investor-portfolio/canonical.ts. The TS
-- module is authoritative (it runs in the sync pipeline + frontend); this SQL fn
-- exists so data migrations and ad-hoc queries can derive the same key in-DB.
--
-- Key format (CAIP-19-aligned):
--   Solana native SOL        -> solana:native:SOL
--   Solana SPL               -> solana:<mint>            (base58, case-preserved)
--   Wrapped SOL (WSOL mint)  -> solana:native:SOL        (MERGED into native SOL)
--   EVM native               -> eip155:<chainId>:native
--   EVM ERC-20               -> eip155:<chainId>:<lowercased contract>
--   Sui/Aptos/Bitcoin/Tron…  -> <namespace>:native:<SYM> or <namespace>:<addr>
-- ============================================================

CREATE OR REPLACE FUNCTION canonical_asset_key(
  p_chain          text,
...
BEGIN
  ...
  IF v_chain = 'solana'
     AND lower(v_addr) = lower('So11111111111111111111111111111111111111112') THEN
...
COMMENT ON FUNCTION canonical_asset_key(text, text, text) IS
  'Investor Intel canonical asset key (mirrors _shared/investor-portfolio/canonical.ts; TS is authoritative).';
