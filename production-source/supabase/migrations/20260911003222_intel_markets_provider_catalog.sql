-- Preserve both provider catalogues and every legacy route. No symbol deduplication.
CREATE INDEX IF NOT EXISTS intel_market_provider_rank ON public.market_assets(source_provider,market_cap_rank ASC NULLS LAST,provider_id);
...
BEGIN
 ...
 v_provider:=CASE WHEN v_requested<>'auto' THEN v_requested
  WHEN (SELECT count(*) FROM public.market_assets WHERE source_provider='coinmarketcap' AND as_of BETWEEN now()-interval '30 minutes' AND now()+interval '30 seconds')>=1000 THEN 'coinmarketcap'
 ...
 SELECT jsonb_build_object('provider',v_provider,'requested',v_requested,'fallback',v_requested='auto' AND v_provider<>'coinmarketcap',
  'available',COALESCE(jsonb_agg(jsonb_build_object('provider',s.source_provider,'assets',s.assets,'oldest',s.oldest,'newest',s.newest)),'[]')) INTO v_catalog
  FROM (SELECT source_provider,count(*) AS assets,min(as_of) AS oldest,max(as_of) AS newest FROM public.market_assets WHERE source_provider IN ('coinmarketcap','coingecko') GROUP BY source_provider)s;
 ...
 SELECT jsonb_build_object('records',COALESCE((SELECT jsonb_agg(to_jsonb(p)) FROM page_rows p),'[]'),
  ...
  'snapshot',(SELECT jsonb_build_object('universeLimit',1000,'universeTotal',(SELECT count(*) FROM public.market_assets WHERE source_provider=$13),
    'universeTruncated',(SELECT count(*) FROM public.market_assets WHERE source_provider=$13)>s.tracked,'enrichmentIncomplete',false,
