...
export function chartSeriesResponse(response:any,servedAt=Date.now()) {
 ...
 const meaning:ChartSource['timestampMeaning']=provider==='coingecko'&&normalized.ohlc?'close':response?.timestampMeaning==='open'&&normalized.ohlc?'open':normalized.ohlc?'unknown':'observation'
