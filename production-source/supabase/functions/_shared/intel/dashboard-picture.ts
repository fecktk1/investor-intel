...
export async function readDashboardPicture(db:any,orgId:string,userId:string,now=new Date()){
 ...
 const [macro,news,flows]=await Promise.all([
  ...
  batch.read('flows',()=>db.from('large_transfer_events').select('chain,canonical_asset_key,symbol,amount,usd_value,threshold_usd,direction,label,observed_at,fetched_at').eq('org_id',orgId).eq('user_id',userId).lte('observed_at',now.toISOString()).order('observed_at',{ascending:false}).limit(9)),
 ])
