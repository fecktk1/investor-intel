...
/** The dashboard's SHARED reads: no member, no workspace. dashboardSourceReads
 * and the public demo's publicDashboardSourceReads run exactly these. */
export function sharedDashboardQueries(db:any){
 return {
  global_news:async()=>{
...
export function dashboardSourceReads(db:any,orgId:string,userId:string,scope:string,chain:string|null){
 ...
 const shared=sharedDashboardQueries(db)
 const sources={
  ...
  global_news:read('global_news',()=>shared.global_news()),
  ...
  exchange_tickers:read('exchange_tickers',()=>shared.exchange_tickers()),
  exchange_signals:read('exchange_signals',()=>shared.exchange_signals()),
  curated_news:read('curated_news',()=>shared.curated_news()),
...
/** The dashboard reads for the public demo: the shared reads exactly as
 * dashboardSourceReads runs them, in the same order, and every per-member read
 * (watchlist, profile, workspace news and narratives, alerts, brief, research,
 * what changed, the personally ranked signal feed) answered EMPTY without a
 * query, as for a visitor who has saved nothing. */
export function publicDashboardSourceReads(db:any){
 const batch=createDashboardReads(),read=batch.read
 const shared=sharedDashboardQueries(db)
 const empty=(name:string,data:any)=>read(name,()=>Promise.resolve({data,error:null}),data===null?'optional':'rows')
 const sources={
  watchlist:empty('watchlist',[]),
  profile:empty('profile',null),
  custom_news:empty('custom_news',[]),
  global_news:read('global_news',()=>shared.global_news()),
  narratives:empty('narratives',[]),
  alerts:empty('alerts',[]),
  brief:empty('brief',null),
  research:empty('research',[]),
  changes:empty('changes',null),
  exchange_tickers:read('exchange_tickers',()=>shared.exchange_tickers()),
  exchange_signals:read('exchange_signals',()=>shared.exchange_signals()),
  curated_news:read('curated_news',()=>shared.curated_news()),
  signal_feed:empty('signal_feed',[]),
 }
 return {...batch,sources}
}
