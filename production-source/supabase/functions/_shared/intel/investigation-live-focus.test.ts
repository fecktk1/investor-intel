...
Deno.test('the tape reports each kind with its metric, unit, clock and evidence metadata',async()=>{
 ...
 eq(db.calls.find(c=>c[0]==='in'&&c[1]==='intel_market_observations')?.[3],['swap_event_usd','liquidity_event_usd','liquidity_usd','unique_traders'])
...
Deno.test('a contract lease is refused until the on-chain tape flag is set, and pausing one always works',async()=>{
 ...
 try{
  ...
  eq(on.calls.find(c=>c[0]==='in'&&c[1]==='intel_market_observations')?.[3],['swap_event_usd','liquidity_event_usd','liquidity_usd','unique_traders'])
