...
export const MCP_TOOLS:ToolSpec[]=[
 ...
 },
 {
  ...
  description:'Premium or discount, in basis points, of each wrapper token against its asset anchor: the volume-weighted median of the liquid wrappers (anchor_kind liquid_wrapper_median). A published_nav anchor is used only for a fund mapped to its own NAV feed; no asset has one mapped today, so expect the median. A wrapper that accrues yield inside its price carries accrual_gap_bps and no premium. wrapper_state says whether a wrapper was liquid enough to count. An empty result is missing coverage, NOT the absence of a premium. For a tokenized stock or ETF, underlying_ref_bps is the same wrapper against the listed share\'s Chainlink price when the wrapper prices were observed (underlying_ref_price, underlying_ref_observed_at, underlying_ref_session), and underlying_ref_within_band true means that gap is inside the feed\'s own update band, so it is not distinguishable from zero and is not a premium. Some wrappers reinvest the share\'s dividends inside the token price (Ondo tokens ending in on, wrapped xStocks): accrual_treatment adjusted means premium_bps and underlying_ref_bps are already of adjusted_price, the price divided by accrual_multiplier (the issuer\'s own on-chain multiplier, accrual_multiplier_source, effective accrual_multiplier_as_of), and raw_premium_bps and underlying_ref_raw_bps are the unadjusted figures; accrual_treatment not_adjusted means no sourced multiplier applied (accrual_reason says why), so the wrapper carries accrual_gap_bps and its underlying_ref_raw_bps includes reinvested dividends and is not a premium. xStocks (symbols ending in x) are read from the same on-chain source (accrual_multiplier_source xstocks_solana_scaled_ui): a wrapped xStock is divided by its multiplier; an xStock at a multiplier of exactly 1 is adjusted by 1; an xStock above 1 is not_adjusted with accrual_reason quote_unit_mixed, because its quote blends per-share venues with on-chain pools that price one raw token, so its accrual_gap_bps may include reinvested dividends.',
  schema:object({
