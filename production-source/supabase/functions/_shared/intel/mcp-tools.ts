...
export const MCP_TOOLS:ToolSpec[]=[
 ...
 },
 {
  ...
  description:'Premium or discount of each wrapper token against its anchor. Reports not_available_yet when this lane has not been built, which is NOT the same as no premium existing.',
  schema:object({
   subject:{type:'string',maxLength:80,pattern:'^[A-Za-z0-9:._-]{1,80}$',description:'One wrapper key. Omit for every wrapper.'},
   limit:LIMIT_PROPERTY(50,25,'Rows to return, at most 50.'),
  }),
  surface:'capture_views',label:'Recorded captures',
  handler:forwardTool('rwa_wrapper_premiums','wrapper_premiums','wrapper premiums',{provider:'investor_intel',endpoint_family:'wrapper premium lane'},['wrapper price and anchor price from the same capture, differenced as a percentage']),
 },
 {
  name:'rwa_liquidity_depth',
  title:'Pools and exitability',
  description:'Pools per token with liquidity, 24h volume, depth at 2 and 5 percent, and the slippage of a given exit size. Reports not_available_yet when this lane has not been built, which is NOT the same as a token having no liquidity.',
  schema:object({
   subject:{type:'string',maxLength:80,pattern:'^[A-Za-z0-9:._-]{1,80}$',description:'One token key. Omit for every token.'},
   limit:LIMIT_PROPERTY(50,25,'Rows to return, at most 50.'),
  }),
  surface:'capture_views',label:'Recorded captures',
  handler:forwardTool('rwa_liquidity_depth','liquidity_depth','liquidity depth',{provider:'investor_intel',endpoint_family:'liquidity depth lane'},['pool reserves and trade sizes from the depth capture']),
 },
 {
  name:'rwa_underlying_registrant',
  title:'The underlying\'s SEC filer',
  description:'SEC filer facts for the asset underlying a token: CIK, registrant name, filer status, SIC, state of incorporation and the latest filing. Reports not_available_yet when this lane has not been built.',
  schema:object({
   subject:SUBJECT_PROPERTY,
