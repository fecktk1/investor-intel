...
import {issuerReviewAt,issuerReviewObservations,recordIssuerReviews,reviewedRelationships,ISSUER_REVIEWED_AT,ISSUER_REVIEW_EXPIRES,EXTENDED_ISSUER_REVIEWED_AT} from './rwa-issuer-evidence.ts'
...
Deno.test('expanded issuer reviews keep separate dates and do not transfer rights to a same-issuer token',async()=>{
 ...
 eq((await issuerReviewObservations([gold],'2026-09-19T04:53:06.000Z')).filter(o=>o.metadata?.reviewVersion==='issuer-review-2'),[])
})
...
Deno.test('source review IDs are stable across reloads and never borrow a market timestamp',async()=>{
 ...
 eq(await issuerReviewObservations([record],iso(now-2000)),[]);eq((await issuerReviewObservations([record],ISSUER_REVIEW_EXPIRES)).filter(o=>o.metadata?.reviewVersion==='issuer-review-1'),[])
})
Deno.test('issuer facts respect recorded knowledge, review expiry and conflicting identities',async()=>{
 ...
 const stale=issuerReviewAt(facts,subject,'38093',Date.parse(ISSUER_REVIEW_EXPIRES));eq(stale.state,'review_expired');eq(stale.facts.length,4);eq(stale.schedule,null);eq(stale.market,null)
