...
import {
  ...
  HOLDING_BATCH_MAX, HOLDING_RUN_LIMIT_MAX, planHoldingResolution, summarizeAnswers,
...
Deno.test('a token the provider did not return is not_found_on_provider', () => {
  ...
  const answers = applyBatchAnswers(
    plan,
    [{ pid: 199, addr: BASE_TOKEN, n: 'Degen', sym: 'DEGEN', p: 0.0121 }],
...
Deno.test('a null, absent or negative price is price_unavailable, never 0', () => {
  ...
  const answers = applyBatchAnswers(
    ...
    [
      ...
      { pid: 199, addr: evm(4), sym: 'D', n: 'Delta' },
    ],
    [{ pid: 199, a: evm(4), p: 0 }],
  )
  const byId = Object.fromEntries(answers.map((a) => [a.holdingId, a]))
...
Deno.test('the identity batch price is used only when the price batch is silent', () => {
  ...
  const answers = applyBatchAnswers(
    plan,
    [{ pid: 199, addr: evm(1), sym: 'A', p: 5 }, { pid: 199, addr: evm(2), sym: 'B', p: 7 }],
    [{ pid: 199, a: evm(1), p: 6 }],
  )
  const byId = Object.fromEntries(answers.map((a) => [a.holdingId, a]))
...
Deno.test('solana subjects keep base58 case and match their own platform id', () => {
  ...
  const answers = applyBatchAnswers(plan, [{ pid: 16, addr: SOL_MINT, sym: 'USDC', n: 'USD Coin', p: 1 }], [{ pid: 16, a: SOL_MINT, p: 1 }])
...
Deno.test('the summary counts every outcome exactly once', () => {
  ...
  const answers = applyBatchAnswers(
    plan,
    [{ pid: 199, addr: evm(1), sym: 'A', p: 2 }, { pid: 199, addr: evm(2), sym: 'B', p: null }],
    [{ pid: 199, a: evm(1), p: 2 }],
  )
  const summary = summarizeAnswers(answers)
  eq(summary, { matched: 2, priced: 1, identityOnly: 1, notFound: 1, unsupported: 1, missingContract: 1, overLimit: 0 })
  eq(answers.length, 5)
  eq(summary.priced + summary.identityOnly + summary.notFound + summary.unsupported + summary.missingContract + summary.overLimit, answers.length)
})
...
Deno.test('a quantity that is not a number never produces a value', () => {
  ...
  const answers = applyBatchAnswers(plan, [], [
    { pid: 199, a: evm(1), p: 3 }, { pid: 199, a: evm(2), p: 3 }, { pid: 199, a: evm(3), p: 3 },
  ])
  const byId = Object.fromEntries(answers.map((a) => [a.holdingId, a]))
