...
Deno.test('all capabilities have a bounded selected-view policy or an explicit entitlement exclusion',()=>{
 for(const name of Object.keys(CMC_CAPABILITIES)){
  ...
  // exchangeListings joined it 2026-09-15: insufficient_entitlement on the Startup key.
  else assert.ok(['rwaPairs','marketPairs','content','community','blockchainStats','exchangeListings'].includes(name))
 }
