...
export type CmcOperatingSettings=Partial<Record<'CMC_ENABLED'|'CMC_CONNECTED_DEMAND_ENABLED'|'CMC_LIVE_ENABLED'|'CMC_LIVE_ONCHAIN_ENABLED'|'CMC_ACCESS_PROFILE'|'CMC_VERIFIED_HACKATHON_PLAN'|'CMC_VERIFIED_BASELINE_PLAN'|'CMC_MONTHLY_CREDIT_CEILING'|'CMC_HACKATHON_EXPIRES_AT'|'CMC_ALLOW_HISTORICAL_RETENTION'|'CMC_HISTORY_RETENTION_DAYS'|'CMC_ALLOW_AI_PROCESSING'|'INTEL_CHART_CMC_PRODUCT_SHARING'|'CMC_SOURCE_POLICY_EXPIRES_AT'|'INTEL_CHART_BINANCE_RETENTION'|'INTEL_CHART_BINANCE_EXPORT'|'INTEL_CHART_BINANCE_SHARING',string>>
// The Binance chart policy sits in the same ledger row so the operator can turn
// it without a deploy, exactly like CoinMarketCap product sharing. A Secret of
// the same name still wins, and no flag grants a right the source terms do not.
const keys=['CMC_ENABLED','CMC_CONNECTED_DEMAND_ENABLED','CMC_LIVE_ENABLED','CMC_LIVE_ONCHAIN_ENABLED','CMC_ACCESS_PROFILE','CMC_VERIFIED_HACKATHON_PLAN','CMC_VERIFIED_BASELINE_PLAN','CMC_MONTHLY_CREDIT_CEILING','CMC_HACKATHON_EXPIRES_AT','CMC_ALLOW_HISTORICAL_RETENTION','CMC_HISTORY_RETENTION_DAYS','CMC_ALLOW_AI_PROCESSING','INTEL_CHART_CMC_PRODUCT_SHARING','CMC_SOURCE_POLICY_EXPIRES_AT','INTEL_CHART_BINANCE_RETENTION','INTEL_CHART_BINANCE_EXPORT','INTEL_CHART_BINANCE_SHARING'] as const
