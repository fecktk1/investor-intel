...
function priceSnapshotFromPack(pack: Any) {
  const m = pack?.market_summary || {}
  return {
    current_price: num(m.current_price),
    change_24h_pct: num(m.change_24h_pct),
    change_7d_pct: num(m.change_7d_pct),
...
async function assemblePack(admin: Any, subject: AssetEvidenceSubject) {
  try {
    return await getOrAssembleAssetEvidencePack(admin, subject, { staleMinutes: 30 })
  } catch (_e) {
    return null
  }
}
...
function evidenceBlock(cards: Any[], max = 14): string {
  return (cards || []).slice(0, max).map((c: Any, i: number) => {
    const partner = (c.event_type === 'partnership' || c.event_type === 'integration')
      ? ` [status:${c.event_status || partnershipMateriality(c).partnership_status}]` : ''
    return `${i + 1}. (${c.event_type}/${c.sentiment || 'neutral'}/${c.materiality})${partner} ${c.title}${c.summary ? ' — ' + String(c.summary).slice(0, 160) : ''}`
  }).join('\n')
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'create') {
      ...
      const result = (subject.canonicalKey || subject.symbol) ? await assemblePack(admin, subject) : null
    ...
    if (action === 'delta') {
      ...
      const { data: thesis, error: tErr } = await user.from('intel_theses').select('*').eq('id', thesisId).single()
      if (tErr || !thesis) return json({ error: 'not_found' }, 404)
      const { data: baseline } = await user.from('intel_thesis_snapshots').select('*').eq('thesis_id', thesisId).eq('snapshot_kind', 'baseline').maybeSingle()
    ...
    if (action === 'quality') {
      ...
      if (body.thesisId) {
        const [t, s, r, e] = await Promise.all([
          user.from('intel_theses').select('*').eq('id', body.thesisId).single(),
    ...
    if (action === 'accept_rule') {
      ...
      const { data: rule, error: rErr } = await user.from('intel_thesis_rules').select('*, thesis:intel_theses(entity_id)').eq('id', ruleId).single()
      if (rErr || !rule) return json({ error: 'not_found' }, 404)
      const triggerType = rule.metric && ['price_move', 'liquidity_drop', 'volume_spike', 'narrative_heat', 'holder_shift'].includes(rule.metric)
        ? rule.metric : 'price_move'
      const config: Record<string, unknown> = {}
      if (rule.threshold != null) config.threshold_pct = rule.threshold
      const { data: alert, error: aErr } = await user.from('intel_alert_rules')
        .insert({ org_id: orgId, user_id: auth.user.id, entity_id: rule.thesis?.entity_id || null, trigger_type: triggerType, config })
        .select('*').single()
      if (aErr) return json({ error: aErr.message }, 400)
      await user.from('intel_thesis_rules').update({ alert_rule_id: alert.id, status: 'active' }).eq('id', ruleId)
      return json({ alert_rule: alert })
    }
    ...
    if (action === 'resolve_status') {
      ...
      const { data: thesis, error: tErr } = await user.from('intel_theses').select('engine_suggested_status').eq('id', thesisId).single()
      if (tErr || !thesis) return json({ error: 'not_found' }, 404)
...
${evidenceBlock(cards) || '(no evidence selected)'}
Draft a disciplined thesis. Bear case must be substantive. Confirmation/invalidation rules must be measurable. If the thesis depends on adoption, include a usage metric in watch_metrics.`
      let out: Any
      try { out = await callCoach(system, userMsg) } catch (e) { return json({ error: (e as Error).message || 'coach_unavailable' }, 200) }
