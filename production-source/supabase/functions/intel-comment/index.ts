...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let { text, usage } = await callText('gpt-5.4-mini', system, `Context (an X post or token note):\n${context.slice(0, 2000)}\n\nWrite the reply.`, apiKey)

    let v = validateSafeLanguage(text)
    let outcome: 'pass' | 'rewrite' | 'block' = 'pass'
    if (!v.ok) {
      const retry = await callText('gpt-5.4-mini', `${system}\nYour previous draft used advice language. Rewrite as research/opinion only.`, `${context.slice(0, 2000)}\n\nPrevious: ${text}`, apiKey)
      text = retry.text; usage = retry.usage; v = validateSafeLanguage(text)
      outcome = v.ok ? 'rewrite' : 'block'
    }
    await recordIntelEvent(supabase, { orgId, userId: auth?.user?.id || null, eventType: 'comment_king', subjectKind: 'general', model: 'gpt-5.4-mini', tokensIn: usage?.prompt_tokens, tokensOut: usage?.completion_tokens, validatorOutcome: outcome, metadata: { replyType } })
    void recordAIUsage(supabase, { orgId, userId: auth?.user?.id || null, provider: 'openai', model: 'gpt-5.4-mini', surface: 'investor_intel', subMode: 'comment_king', providerUsage: usage, status: 'success' })
